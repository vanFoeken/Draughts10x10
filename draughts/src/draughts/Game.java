 ...
 * hintBoard -> moveable or selected piece, mouse input
 * 
 * boards -> previous player boards -> undo
 * 
 * player -> player color
 * 
 * pieces -> pieces per color
 * moves -> moves per piece
 * maxCapture -> captures per move
 * 
...
final class Game extends JLayeredPane implements ActionListener {
                ...
                for (int pressed = 0; pressed < BOARD10X10.tile.length; pressed++) {
                    if (BOARD10X10.tile[pressed].contains(e.getPoint())) {
                        ...
                        //2 pressed = occupied
                            ...
                            repaint();
                        //3 selected != NOT_SELECTED (tile[pressed] = EMPTY)
                        } else if (selected != NOT_SELECTED) {
    ...
    final private class BoardMove extends Component implements Runnable {
        ...
        public void run() {
            ...
            //promotion
            if (piece == PAWN[color] && ((color == WHITE && to < GRID / 2) || (color == BLACK && to >= (GRID - 1) * GRID / 2))) {
                piece = KING[color];
            }
