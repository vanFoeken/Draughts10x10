...
final class Game extends JLayeredPane implements ActionListener {
    ...
    Game(int player) {
        ...
        INFO.setText("Player is " + COLOR[player]);//text disappears after first player move
    ...
    private void turn(int color) {
                        ...
                        if (pieces[opponent].contains(step) && vertical.hasNext(step)) {
                            ...
                            if (board[step] == EMPTY) {//capture is legal
                                ...
                                do {
                                    ...
                                    for (int to : destination) {//empty tile(s)
        ...
        if (BOARD10X10.isAncestorOf(this)) {//continue only if this (game) is on BOARD10X10; prevent enable ARROW by new game during AI search
            if (moves.isEmpty()) {//game over
                INFO.setText(COLOR[opponent] + " is Winner");
    ...
    private class BoardMove extends Component implements Runnable {
        ...
        public void run() {
            ...
            for (int step : pieceBoard.getMove()) {//<<captured>, to>
                ...
                } while (direction.x * (x(step) - x(to)) != direction.y * (y(to) - y(step)));//x!=-y
