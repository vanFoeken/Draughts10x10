 ...
 * 1 level = 2 ply (moves) -> alfa (max) & beta (min)
 * level=0 continue while maxCapture > 0
 * value -> -/+ maxCapture
 * 
 * enum Node -> alfa beta
...
final class MinMax extends HashMap<String, Integer> {
    ...
    private int valueOf(char[] board, long turn, long opponent, MinMax minMax, int[] alfaBeta, int value, int level) {
        //1 moves maxCapture
        ...
        if (moves.isEmpty()) {//game over
            ...
            level -= node.ordinal();//ALFA0, BETA-1
        ...
        for (int from : moves.keySet()) {
            ...
            for (long move : moves.get(from)) {
                ...
                for (long destination = move ^ captures; destination != 0l; destination ^= Long.lowestOneBit(destination)) {//empty
                    ...
                    if (!containsKey(key)) {
                        put(key, minMax.valueOf(key.toCharArray(), opponent ^ captures, turn ^ (1l << from ^ 1l << to), this, alfaBeta.clone(), value, level));
                    }
    ...
    static ArrayList<Integer> getAIMove(int ai, char[] board, HashSet<Integer>[] pieces, HashMap<Integer, ArrayList<Integer>[]> moves, int maxCapture, int level) {
        ...
        for (int from : moves.keySet()) {
            ...
            for (ArrayList<Integer> move : moves.get(from)) {//<<captured>, to>
                ...
                int min = new MinMax(Node.BETA, player).valueOf(getBoard(ai, board.clone(), piece, move, to), opponent ^ captures, turn ^ (1l << from ^ 1l << to), new MinMax(Node.ALFA, ai), new int[] {-INFINITY, INFINITY}, maxCapture, level);//beta
