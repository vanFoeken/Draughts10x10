...
final class MinMax extends HashMap<String, Integer> {
    ...
    private static enum Node {
        ...
        abstract int valueOf(int value);//maxCapture
    }
    ...
    private static enum Diagonal {
        ...
        static {
            for (int i = 0; i < LEFT_RIGHT.length; i++) {
                ...
                for (int bit = COLUMN - 1 - Math.min(i, COLUMN - 1) + i / COLUMN * COLUMN + Math.max(0, i - COLUMN) * GRID; Long.bitCount(LEFT_RIGHT[i]) < 1 + (Math.min(i, COLUMN - 1) - Math.max(0, i - COLUMN)) * 2; bit += COLUMN + 1 - bit / COLUMN % 2) {
                    LEFT_RIGHT[i] ^= 1l << bit;
                }
            }
            ...
            for (int i = 0; i < RIGHT_LEFT.length; i++) {
                ...
                for (int bit = Math.min(i, COLUMN - 1) + Math.max(0, i - (COLUMN - 1)) * GRID; Long.bitCount(RIGHT_LEFT[i]) < 2 + (Math.min(i, COLUMN - 1) - Math.max(0, i - (COLUMN - 1))) * 2; bit += COLUMN - bit / COLUMN % 2) {
                    RIGHT_LEFT[i] ^= 1l << bit;
                }
            }
        }
    ...
    static ArrayList<Integer> getAIMove(int ai, char[] board, HashSet<Integer>[] pieces, HashMap<Integer, ArrayList<Integer>[]> moves, int maxCapture, int depth) {
        ...
        for (int from : moves.keySet()) {
            ...
            for (ArrayList<Integer> move : moves.get(from)) {
                ...
                int min = new MinMax(Node.MIN, player).valueOf(getBoard(ai, board.clone(), piece, move, to), opponent ^ captures, turn ^ (1l << from ^ 1l << to), new MinMax(Node.MAX, ai), new int[] {ALFA, BETA}, maxCapture, depth);
