    ...
    static ArrayList<Integer> getAIMove(int ai, char[] board, HashSet<Integer>[] pieces, HashMap<Integer, ArrayList<Integer>[]> moves, int maxCapture) {
        ...
        for (int from : moves.keySet()) {
            ...
            for (ArrayList<Integer> move : moves.get(from)) {//<<captured>, to>
                ...
                int min = new MinMax(Node.MIN, player).valueOf(getBoard(ai, board.clone(), piece, move, to), opponent ^ captures, turn ^ (1l << from ^ 1l << to), new MinMax(Node.MAX, ai), new int[] {ALFA, BETA}, maxCapture, AI.getValue());//beta
                
                if (min >= max) {//beta>=alfa
                    if (min > max) {//beta>alfa
                        alfaMoves.clear();                        
