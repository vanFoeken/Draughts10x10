...
final class MinMax extends HashMap<String, Integer> {
    ...
    static ArrayList<Integer> getAIMove(int ai, char[] board, HashSet<Integer>[] pieces, HashMap<Integer, ArrayList<Integer>[]> moves, int maxCapture, int level) {
        ...
        MinMax min = new MinMax(Node.BETA, player);
        MinMax max = new MinMax(Node.ALFA, ai);
...
//                int beta = new MinMax(Node.BETA, player).valueOf(getBoard(ai, board.clone(), piece, move, to), opponent ^ captures, turn ^ (1l << from ^ 1l << to), new MinMax(Node.ALFA, ai), new int[] {BETA, ALFA}, maxCapture, level);//beta
