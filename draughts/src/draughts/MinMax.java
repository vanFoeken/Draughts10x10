 ...
 * 1 level = 2 ply (moves) -> 1 alfa (max) & 1 beta (min)
...
final class MinMax extends HashMap<String, Integer> {
    ...
    private int valueOf(char[] board, long turn, long opponent, MinMax minMax, int[] alfaBeta, int value, int depth) {
        ...
        if (moves.isEmpty()) {//game over
            ...
            depth -= node.ordinal();//ALFA0, BETA-1
    ...
    static ArrayList<Integer> getAIMove(int ai, char[] board, HashSet<Integer>[] pieces, HashMap<Integer, ArrayList<Integer>[]> moves, int maxCapture, int depth) {
        ...
        for (int from : moves.keySet()) {
            ...
            for (ArrayList<Integer> move : moves.get(from)) {//<<captured>, to>
                ...
                int min = new MinMax(Node.BETA, player).valueOf(getBoard(ai, board.clone(), piece, move, to), opponent ^ captures, turn ^ (1l << from ^ 1l << to), new MinMax(Node.ALFA, ai), new int[] {BETA, ALFA}, maxCapture, depth);//beta
